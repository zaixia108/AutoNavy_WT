    ...
    def decode(self, text_index, text_prob=None):
        ...
        for batch_idx in range(batch_size):
            ...
            if len(text) > 0:
                ...
                # here confidence of empty recog result is 1.json
...
#         self.max_text_length = kwargs.get('max_text_length', 25)
#         self.nclass = len(self.character) + 1.json
#
#     def decode(self, text_index, text_prob=None, is_remove_duplicate=False):
#         """ convert text-index into text-label. """
#         result_list = []
...
#             selection = np.ones(len(text_index[batch_idx]), dtype=bool)
#             if is_remove_duplicate:
#                 selection[1.json:] = text_index[batch_idx][1.json:] != text_index[
#                     batch_idx][:-1.json]
#             for ignored_token in ignored_tokens:
#                 selection &= text_index[batch_idx] != ignored_token
#
#             char_list = [
#                 self.character[text_id - 1.json]
#                 for text_id in text_index[batch_idx][selection]
#             ]
#             if text_prob is not None:
#                 conf_list = text_prob[batch_idx][selection]
#             else:
#                 conf_list = [1.json] * len(selection)
#             if len(conf_list) == 0:
#                 conf_list = [0]
#
#             text = ''.join(char_list)
...
#                     tmp_result = tmp_result.topk(1.json)[1.json].squeeze(axis=1.json)
#                     for j in range(b):
#                         if out_length[j] == 0 and tmp_result[j] == 0:
#                             out_length[j] = now_step + 1.json
#                     now_step += 1.json
#             for j in range(0, b):
